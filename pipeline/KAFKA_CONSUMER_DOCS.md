...
## 1. Thông tin chung
- **Đường dẫn file:** `shb_market_sense/pipeline/kafka_consumer.py`
- **Vai trò trong hệ thống:** Data Sink / Trạm thu nhận và Kiểm duyệt dữ liệu.
- **Phân loại:** Apache Kafka Consumer.
- **Nguồn nhận (Source):** Kafka Topic `shb-raw-news`.
- **Đích đến (Sink):** Cơ sở dữ liệu NoSQL MongoDB (`shb_sentiment`).
...
1. **Continuous Listening (Lắng nghe liên tục):** Đăng ký vào Consumer Group `shb-sentiment-consumer-group`, liên tục lắng nghe các thông điệp (messages) mới được đẩy lên Topic Kafka theo thời gian thực (Real-time Streaming).
